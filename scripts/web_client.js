...
var WebClient = {
    ...
    on_roster: function (iq) {
        $(iq).find('item').each(function () {
            ...
            var jid_id = WebClient.jid_to_id(jid);
            var contact = $("<li id='" + jid_id + "'>" +
                            "<div class='roster-contact offline'>" +
                            "<div class='roster-name'>" + name +
                            "</div><div class='roster-jid'>" + jid +
                            "</div></div></li>");
            WebClient.insert_contact(contact);
        });

        // set up presence handler and send initial presence
        WebClient.connection.addHandler(WebClient.on_presence, null, "presence");
    ...
    on_presence: function (presence) {
        ...
        var jid_id = WebClient.jid_to_id(from);

        if (ptype === 'subscribe') {
            // populate pending_subscriber, the approve-jid span, and
            // open the dialog
            WebClient.pending_subscriber = from;
        ...
        $(iq).find('item').each(function () {
            ...
            if (sub === 'remove') {
                ...
                if ($('#' + jid_id).length > 0) {
                    ...
                    WebClient.insert_contact($(contact_html));
                }
            }
        });
    ...
    insert_contact: function (elem) {
        ...
        if (contacts.length > 0) {
            ...
            contacts.each(function () {
                var cmp_pres = WebClient.presence_value(
...
$(document).ready(function () {
    ...
    $('.chat-input').live('keypress', function (ev) {
        ...
        if (ev.which === 13) {
            ...
            WebClient.connection.send(message);
            ...
            $(this).parent().find('.chat-messages').append(
                ...
                body +
                "</span></div>");
            WebClient.scroll_chat(WebClient.jid_to_id(jid));
            ...
            if (!composing) {
                ...
                WebClient.connection.send(notify);
    ...
    $('#disconnect').click(function () {
        WebClient.connection.disconnect();
...
$(document).bind('connect', function (ev, data) {
    ...
    WebClient.connection = conn;
});

$(document).bind('connected', function () {
    var iq = $iq({type: 'get'}).c('query', {xmlns: 'jabber:iq:roster'});
    WebClient.connection.sendIQ(iq, WebClient.on_roster);

    WebClient.connection.addHandler(WebClient.on_roster_changed,
                              "jabber:iq:roster", "iq", "set");

    WebClient.connection.addHandler(WebClient.on_message,
                              null, "message", "chat");
});
